...
debug = function(message) {
  if (LaunchpadProMK3.debug) { console.log(red + "DEBUG: " + reset + message) }
}
...
LaunchpadProMK3.Deck = function (deckNumber) {
  ...
  debug("### Deck objects instantiation:  deckNumber " + deckNumber + "\n")
  ...
  for (let i = 1; i <= 16; i++) {
    ...
    let padAddress = LaunchpadProMK3.mainpadAddresses[padAddressIndex]
    this.i = i

    debug("# hotcueButtons:   deck " + deckNumber + "/" + this.currentDeck + ",  i " + i + ",  padAddressIndex " + padAddressIndex + ",  padAddress " + padAddress + "\n")
  ...
  for (let sidepad = 1; sidepad <= 4; sidepad++) {
    ...
    debug("# deck " + deckNumber + ", sidepad " + sidepad + ",  sidepadAddressIndex " + sidepadAddressIndex + ",  padAddress " +  padAddress + ",  sidepadControlName " + sidepadControlName + "\n")
...
LaunchpadProMK3.connectHotcues = function() {
  ...
  for (let deck = 1; deck <= 4; deck++) {
    debug("\x1b[33mconnectHotcues decks:\x1b[0m " + deck + "\n")
...
LaunchpadProMK3.updateHotcueLights = function(deck) {
  ...
  debug("## updateDeckLights:   deck " + deck + ",   hotcutOffset " + hotcueOffset + "\n")
  for (var i = 0; i < 16; i++) {
    ...
    if (cueEnabled) {
      ...
      LaunchpadProMK3.sendRGB(LaunchpadProMK3.mainpadAddresses[i + hotcueOffset], rgb[0]/2, rgb[1]/2, rgb[2]/2);
    } else {
      // LaunchpadProMK3.turnOffPad(LaunchpadProMK3.mainpadAddresses[i + hotcueOffset]);
    }
  }
}
