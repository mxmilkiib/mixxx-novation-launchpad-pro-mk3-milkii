...
LaunchpadProMK3.initExtras = function () {
  ...
  midi.makeInputHandler(0xB0, LaunchpadProMK3.buttons.allEffectsEnabled, (channel, control, value, status, group) => {
    if (value === 0x7F) {  // Button pressed
      ...
      // Original effects toggle functionality
      LaunchpadProMK3.allEffectsEnabled = !LaunchpadProMK3.allEffectsEnabled;
      
      // List of effect units to control (you can add more effect units here)
      var effectUnits = [
      ...
      // For each effect unit, control individual effects within each unit
      for (var unit = 0; unit < effectUnits.length; unit++) {
        for (var effectNum = 1; effectNum <= 3; effectNum++) {
          var effectGroup = effectUnits[unit] + "_Effect" + effectNum;
          var effectControl = "enabled";
          engine.setValue(effectGroup, effectControl, LaunchpadProMK3.allEffectsEnabled ? 1 : 0);
        }
      }
